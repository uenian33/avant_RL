...
from keras.layers import Input, BatchNormalization, Activation, Conv3D, Dropout, Concatenate, AveragePooling3D, MaxPooling3D, Dense, Flatten, GlobalAveragePooling2D, MaxPool3D
...
def DenseNet3D(input_shape, growth_rate=32, block_config=(6, 12, 24, 16),
               num_init_features=64, bn_size=4, drop_rate=0, num_classes=5, use_pretrained=True):
...
def densenet161_3D_DropOut(input_shape, nb_classes, use_pretrained=True):
    model = DenseNet3D(input_shape, growth_rate=48, block_config=(
        6, 12, 36, 24), num_init_features=96, drop_rate=0.6, num_classes=nb_classes, use_pretrained=use_pretrained)
    return model
...
def xception_classifier(input_shape, nb_classes, use_pretrained=True):
    inp_2d = (Input(shape=(224, 224, 3), name='2d_input'))
    pretrained_xcept = xception.Xception(include_top=False, weights='imagenet', input_tensor=inp_2d,
                                         input_shape=(224, 224, 3), pooling='avg', classes=nb_classes)
    print(pretrained_xcept.output.shape)
    #x = GlobalAveragePooling2D(name='avg_pool_densnet2d')(pretrained_xcept.output)
    x = Dense(1024, activation='relu')(pretrained_xcept.output)

    x = Dropout(0.65)(x)
    x = Dense(512, activation='relu')(x)
    x = Dropout(0.35)(x)
    out = Dense(nb_classes, activation='softmax')(x)
    model = Model(inputs=[inp_2d], outputs=[out])

    return model


def c3d_model(input_shape, nb_classes, feature=False):
    #input_shape = (112,112,16,3)
    weight_decay = 0.005
    print(input_shape)
    inp_3d = (Input(shape=input_shape, name='3d_input'))
    x = Conv3D(64, (3, 3, 3), strides=(1, 1, 1), padding='same',
               activation='relu', kernel_regularizer=l2(weight_decay))(inp_3d)
    ...
    if feature:
        ...
        x = Dense(8, activation='linear', kernel_regularizer=l2(weight_decay))(x)
        x = Dropout(0.5)(x)
...
def c3d_model_feature(input_shape, nb_classes):
    #input_shape = (112,112,16,3)
    weight_decay = 0.005
    print(input_shape)
    ...
    f = Dense(8, activation='linear', kernel_regularizer=l2(weight_decay))(x)
    x = Dropout(0.5)(f)
    ...
    model = Model(inputs=[inp_3d], outputs=[f, out])
    return model
